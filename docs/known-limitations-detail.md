...
> 这是 [issue #4「已知限制汇总」](https://github.com/shiinayane/BiliKit-Web/issues/4) 的**详细技术版**。公开 issue 只留给用户看的简短说明；这里保留完整的排查过程、死胡同、数据与方法，便于日后不必重走弯路。改动限制结论时，两处一起更新。
...
- **BiliKit-Web 净增约 +300MB（峰值 +556MB），是少数派**，且分散在 WebCore 堆 / JS 堆 / 合成面；后续深入对照确认，其中很大一部分与「完整视频页 iframe 的首次生命周期」有关，见下节；
...
用户在同一台机器上追加观察：关闭全部脚本时，B 站网页稳定约 **1.2GB**，打开普通视频页约 **1.4GB**；开启当前 BiliKit-Web 后虽然绝对值约稳定在 **1.6GB**、偶尔到 **2GB**，但峰值会回落，连续使用时反而没有原生页面增长得快。

这组数据支持“当前实现是较高但有界的平台，而非按操作次数持续泄漏”，也与 Feed 窗口化、媒体有界释放和抽屉跨视频换 Document 的设计方向一致。但它不是严格匹配时长、推荐内容与操作序列的新一轮 A/B，不能据此宣称“脚本会降低 Safari 内存”；更稳妥的结论是：**绝对 footprint 与增长斜率是两件事，当前没有观察到 BiliKit-Web 导致更快的单调增长。**
